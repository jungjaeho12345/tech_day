...
description: >
  기사 작성기 하네스 워크플로 — /news produce/plan/verify 라우팅,
...
| 차원 | 의미 (뉴스 도메인 적용) |
|------|---------------------|
| **Design Quality** | 디자인 토큰(`--yh-blue` `#0A4DA6` 등) 정합, 신문형 레이아웃 밀도, 색 규칙(제목 파란색 / 부제목 빨간색 / 본문 검정) 준수 |
...
- **수집/배부 시스템** — CLAUDE.md "현재 구현 범위는 기사 작성기만 진행".
