...
describe('SPEC-NEWS-REVISE-008 §1 REQ-LOCK-RETENTION — 편집 잠금 탭 생존 유지', () => {
  ...
  // L-2 (보안 리뷰) — sessionStorage 생존 신호 무결성: 탭 항목의 editStartedAt 타임스탬프가 서버 stale
  // 창(30분)을 넘기면 editTabSurvives 가 만료로 판정해, 조작·동결된 sessionStorage 가 잠금을 무한정
  // 고정하지 못하게 한다(unmount 시 해제). 타임스탬프가 신선하면 종전대로 유지된다.
  function seedEditTabWithTimestamp(articleId, editStartedAt, tabId = 't2') {
    sessionStorage.setItem(TABS_KEY, JSON.stringify({
      tabs: [{ id: 't1', editArticleId: null }, { id: tabId, editArticleId: articleId, editStartedAt }],
      activeId: tabId,
      seq: 2,
    }));
  }

  it('L-2: editStartedAt 이 30분 TTL 을 초과한 (동결된) 생존 신호는 unmount 가 락을 해제한다', async () => {
    // 31분 전 타임스탬프 — 서버 stale 창(30분)을 초과 → 더 이상 잠금 유지 신호로 인정하지 않는다.
    seedEditTabWithTimestamp('AKR-TTL-1', Date.now() - (31 * 60 * 1000));
    const lockArticle = vi.fn().mockResolvedValue({ ok: true });
    const unlockArticle = vi.fn().mockResolvedValue({ ok: true, released: true });
    const queryArticles = vi.fn().mockResolvedValue([{ articleId: 'AKR-TTL-1', markupVersion: markupFor('본문') }]);
    const { unmount } = renderCtrl(
      createFakeModel({ lockArticle, unlockArticle, queryArticles }),
      { editArticleId: 'AKR-TTL-1', draftKey: 'newsroom.writeDraft.t2' },
    );

    await waitFor(() => expect(lockArticle).toHaveBeenCalledWith('AKR-TTL-1'));
    unmount();
    expect(unlockArticle).toHaveBeenCalledWith('AKR-TTL-1');
  });

  it('L-2: editStartedAt 이 신선하면(TTL 내) 종전대로 unmount 가 락을 유지한다', async () => {
    seedEditTabWithTimestamp('AKR-TTL-2', Date.now() - (60 * 1000)); // 1분 전 — TTL 내.
    const lockArticle = vi.fn().mockResolvedValue({ ok: true });
    const unlockArticle = vi.fn().mockResolvedValue({ ok: true, released: true });
    const queryArticles = vi.fn().mockResolvedValue([{ articleId: 'AKR-TTL-2', markupVersion: markupFor('본문') }]);
    const { unmount } = renderCtrl(
      createFakeModel({ lockArticle, unlockArticle, queryArticles }),
      { editArticleId: 'AKR-TTL-2', draftKey: 'newsroom.writeDraft.t2' },
    );

    await waitFor(() => expect(lockArticle).toHaveBeenCalledWith('AKR-TTL-2'));
    unmount();
    expect(unlockArticle).not.toHaveBeenCalled();
  });
