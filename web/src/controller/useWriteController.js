...
/**
 * @param {AuthUser} user
 * @param {{ editArticleId?: string }} [options] when editArticleId is set, the controller first ACQUIRES
 *   the edit lock (lock-before-load), then loads that article on mount (markupVersion + common fields)
 *   and saves PUT (update) instead of POST (create). A 409 conflict blocks the load and bounces to 조회.
 */
export function useWriteController(user, options = {}) {
  ...
  // 잠금 획득 플래그 — true 인 동안만 unmount/beforeunload/logout 에서 unlockArticle 을 호출한다.
  // 409 차단으로 진입했거나 액션 성공으로 서버 auto-release 가 일어난 뒤에는 false 로 두어 이중 해제를 막는다.
  ...
  // SPEC-EDIT-LOCK-001 REQ-EDIT-LOCK — 보유 잠금 best-effort 해제(공유 경로). 획득 플래그가 켜져 있을 때만
  // unlockArticle(id) 를 호출하고 즉시 플래그를 끈다(중복 해제 방지). beforeunload/unmount/logout 이 공유한다.
  const releaseHeldLock = useCallback(() => {
    if (!editArticleId) return;
    if (!acquiredLockRef.current) return;
    acquiredLockRef.current = false;
    try {
      const ret = model.unlockArticle(editArticleId);
      if (ret && typeof ret.catch === 'function') ret.catch(() => {});
    } catch {
      // 해제는 best-effort — 언로드/로그아웃 경로를 막지 않는다.
    }
  }, [editArticleId, model]);

  // beforeunload(탭 닫기) — 보유 잠금을 best-effort 해제한다(unlockArticle 은 keepalive 로 언로드 중 전송 보장).
  useEffect(() => {
    if (!editArticleId) return undefined;
  ...
  const submitAction = useCallback(async (action) => {
    ...
    try {
      let id = articleId;
      if (action === 'send' || action === 'hold') {
        // hold follows the same "save then apply" path as send so that a brand-new (A-DRAFT) article
        // is first persisted before the lifecycle transition is attempted (news.md 96~99 rows).
        // SPEC-EDIT-LOCK-001: 서버 PUT lock-guard 는 x-session-id 세션 holder 로 판단하므로 DTO 에
        // sessionId 를 끼워 넣지 않는다(불필요 — 폐기).
        const dto = assembleDto();
        const saved = await model.saveArticle(articleId, dto);
