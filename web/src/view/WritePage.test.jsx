...
describe('WritePage action-button visibility (news.md 기사 작성 페이지 내 버튼)', () => {
  ...
  // SPEC-NEWS-REVISE-001 AC-Z-3 (원): Z권한이라도 status가 RDS가 아니면 송고/보류/KILL 비표시였다.
  // SPEC-NEWS-REVISE-011 (델타 B, 2026-06-10 사용자 승인): DPS 기사 고침/포털고침 시 R/D/Z 에게 송고/보류를
  // 노출한다(KILL 은 여전히 비표시). 따라서 DPS 의 "all hidden" 단언은 신규 승인 동작과 모순되어, DPS 컨텍스트의
  // 새 진실(송고/보류 노출 + KILL 비표시)로 갱신한다. RDS-외 비표시 불변(SPEC-001 D-1)은 RRH/RRK 등 다른 상태에서
  // 계속 유효하다(DPS 만 델타 B 로 예외화됨).
  it('AC-Z-3 (SPEC-NEWS-REVISE-011 갱신): role Z + DPS: 송고/보류 노출, KILL 비표시', async () => {
  ...
  // SPEC-NEWS-REVISE-011 (델타 B, 2026-06-10 사용자 승인): DPS 기사 고침/포털고침 시 R/D/Z 에게 송고/보류를
  // 노출한다(KILL 비표시). 종전 "all three hidden" 단언은 신규 승인 동작과 모순되어 새 진실로 갱신한다.
  it('edit-loaded DPS article (SPEC-NEWS-REVISE-011): role R 도 송고/보류 노출, KILL 비표시', async () => {
    ...
    // Wait for the edit-load to settle (loaded author appears), then assert the DPS button matrix.
