...
function renderEmbed(embed) {
  ...
  if (embed.type === 'video') {
    ...
    // javascript: 등 비-HTTP(S) 프로토콜 URL은 href 링크를 생성하지 않는다 (XSS 방지).
    const safeUrl = embed.url && /^https?:\/\//.test(embed.url) ? embed.url : null;
    const link = safeUrl
      ? `<a class="yh-detail__embed-link" href="${escapeHtml(safeUrl)}" rel="noreferrer">${escapeHtml(safeUrl)}</a>`
      : '';
