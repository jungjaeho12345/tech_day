...
describe('ViewPage four menus (REQ-FE-VIEW-004..008)', () => {
  ...
  it('AC-7.3: 개인별 수정 — 본인 작성 + 상태 RDS/RRK만 (v0.4.0)', async () => {
    ...
    expect(queryArticles).toHaveBeenCalledWith({ author: 'd1', status: 'RDS,RRK' });
    expect(await screen.findByText('mine')).toBeInTheDocument();
  });

  it('AC-7.4: 데스크 미송고 queries RDS+DDH and renders the 7 fixed columns', async () => {
    const user = userEvent.setup();
    ...
    // 7 columns: 기사아이디 / 제목 / 작성자 / 수정자 / 작성시간 / 수정시간 / LockYN.
    ...
    // Header row labels the 7 columns.
    expect(screen.getByTestId('desk-header')).toHaveTextContent('LockYN');
    // 상태 배지/인라인 편집 버튼은 데스크 미송고 컬럼 스펙에 없다 (편집은 우클릭 메뉴에 유지).
    expect(screen.queryByTestId('article-status')).not.toBeInTheDocument();
...
describe('ViewPage list columns + time-desc order (REQ-FE-VIEW-011 v0.4.0 — 전 메뉴 7컬럼)', () => {
  const twoRows = [
    {
      articleId: 'OLD',
      title: 'older art',
  ...
  it('renders the unified 7 columns (no status badge) and sorts time DESC in 부서별 작성', async () => {
    const u = userEvent.setup();
    ...
    // 7개 컬럼: 기사아이디/제목/작성자/수정자/작성시간/수정시간/LockYN — 시간 내림차순.
    ...
    // v0.4.0: 상태 배지는 어느 메뉴에도 없다.
    expect(screen.queryAllByTestId('article-status')).toHaveLength(0);
    // 공유 7컬럼 헤더가 데스크 미송고 외 메뉴에서도 표시된다.
    expect(screen.getByTestId('desk-header')).toHaveTextContent('LockYN');
