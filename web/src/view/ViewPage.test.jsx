...
describe('ViewPage pagination — 10 per page + dense list (news.md 기사 조회페이지)', () => {
  ...
  // 23 distinct rows (descending createdAt) for the exact 1 / 3 indicator and last-page remainder checks.
  const twentyThreeRows = Array.from({ length: 23 }, (_, i) => {
    const n = 23 - i; // n = 23 (newest) .. 1 (oldest)
    ...
    return {
      ...
      modifier: '수정',
      status: 'RDS',
      createdAt: `2026-05-${k}T08:00:00Z`,
    };
  });

  // AC-PAGE-1 (SPEC-NEWS-REVISE-010): 23건 조회 시 첫 페이지 정확히 10행 + 지시자 `1 / 3`.
  it('AC-PAGE-1: 23건 조회 → 첫 페이지 10행 + 지시자 1 / 3', async () => {
    const queryArticles = vi.fn().mockResolvedValue(twentyThreeRows);
    renderView(createFakeModel({ queryArticles }));
    await screen.findByText('bart-23'); // newest first
    expect(screen.getAllByTestId('article-id')).toHaveLength(10);
  ...
  // AC-PAGE-2 보강 (SPEC-NEWS-REVISE-010): 3페이지에서 마지막 3행만 표시 + 지시자 `3 / 3`.
  it('AC-PAGE-2: 23건 → 마지막(3) 페이지에 나머지 3행만 표시', async () => {
    const queryArticles = vi.fn().mockResolvedValue(twentyThreeRows);
    const user = userEvent.setup();
    renderView(createFakeModel({ queryArticles }));
    await screen.findByText('bart-23');
    await user.click(screen.getByTestId('page-next')); // → 2 / 3
    expect(screen.getByTestId('page-indicator')).toHaveTextContent('2 / 3');
    expect(screen.getAllByTestId('article-id')).toHaveLength(10);
    await user.click(screen.getByTestId('page-next')); // → 3 / 3
    expect(screen.getByTestId('page-indicator')).toHaveTextContent('3 / 3');
    // 23 = 10 + 10 + 3 → last page holds exactly the 3 oldest rows.
    expect(screen.getAllByTestId('article-id')).toHaveLength(3);
    expect(screen.getByText('bart-03')).toBeInTheDocument();
    expect(screen.getByText('bart-01')).toBeInTheDocument();
    expect(screen.getByTestId('page-next')).toBeDisabled();
  });

  // AC-PAGE-3 / EC-1 (SPEC-NEWS-REVISE-010): 정확히 10건이면 단일 페이지 — 페이지 컨트롤(yh-pagination) 미노출.
  it('AC-PAGE-3 / EC-1: 정확히 10건이면 yh-pagination 컨트롤이 노출되지 않는다', async () => {
    const tenRows = twelveRows.slice(0, 10); // exactly PAGE_SIZE rows
    const queryArticles = vi.fn().mockResolvedValue(tenRows);
    const { container } = renderView(createFakeModel({ queryArticles }));
    await screen.findByText(tenRows[0].title);
    expect(screen.getAllByTestId('article-id')).toHaveLength(10);
    // 10개 이하 → 페이지 이동 컨트롤(이전/다음/지시자)이 없다.
    expect(container.querySelector('.yh-pagination')).toBeNull();
  ...
  // AC-PAGE-3 보강 (SPEC-NEWS-REVISE-010): 10건 미만(5건)도 컨트롤 미노출.
  it('AC-PAGE-3: 10건 미만(5건)이면 yh-pagination 컨트롤이 노출되지 않는다', async () => {
    const fiveRows = twelveRows.slice(0, 5);
    const queryArticles = vi.fn().mockResolvedValue(fiveRows);
    const { container } = renderView(createFakeModel({ queryArticles }));
    await screen.findByText(fiveRows[0].title);
    expect(container.querySelector('.yh-pagination')).toBeNull();
  });

  // EC-2 (SPEC-NEWS-REVISE-010): 0건 → 빈 목록, 컨트롤 미노출, 지시자 오류 없음(경계 안전).
  it('EC-2: 0건이면 빈 목록 + yh-pagination 컨트롤 미노출 (경계 오류 없음)', async () => {
    const queryArticles = vi.fn().mockResolvedValue([]);
    const { container } = renderView(createFakeModel({ queryArticles }));
    await act(async () => {}); // settle the initial query
    expect(screen.queryAllByTestId('article-id')).toHaveLength(0);
    expect(container.querySelector('.yh-pagination')).toBeNull();
    expect(screen.queryByTestId('page-indicator')).not.toBeInTheDocument();
    // 헤더도 행이 없으면 노출되지 않는다 (pageItems.length > 0 가드).
    expect(screen.queryByTestId('desk-header')).not.toBeInTheDocument();
  });

  // AC-PAGE-5 (SPEC-NEWS-REVISE-010): 3페이지를 보던 중 목록이 8건(1페이지)으로 축소 → 현재 페이지가 마지막 유효 페이지로 보정.
  it('AC-PAGE-5: 페이지 초과 시 마지막 유효 페이지로 보정 (목록 축소)', async () => {
    const user = userEvent.setup();
    // 실시간 갱신을 흉내내기 위해 SSE 신호로 재조회를 트리거한다: 첫 조회 23건, change 후 8건.
    const queryArticles = vi.fn()
      .mockResolvedValueOnce(twentyThreeRows)
      .mockResolvedValueOnce(twelveRows.slice(0, 8)); // 8 rows = 1 page worth
    const { model } = renderView(createFakeModel({ queryArticles }));
    await screen.findByText('bart-23');
    // 3페이지로 이동.
    await user.click(screen.getByTestId('page-next')); // 2 / 3
    await user.click(screen.getByTestId('page-next')); // 3 / 3
    expect(screen.getByTestId('page-indicator')).toHaveTextContent('3 / 3');
    // 실시간 갱신: 목록이 8건으로 축소되면 페이지 컨트롤이 사라지고(<=10건) 현재 페이지가 보정된다.
    await act(async () => { model.__emit({ type: 'status', articleId: 'B-23' }); });
    await screen.findByText(twelveRows[0].title);
    // 8건 → 단일 페이지 → 컨트롤 자체가 사라진다(보정 결과: page 1 로 clamp 되며 컨트롤 미노출).
    expect(screen.queryByTestId('page-indicator')).not.toBeInTheDocument();
    expect(screen.getAllByTestId('article-id')).toHaveLength(8);
  });

  // AC-PAGE-6 (SPEC-NEWS-REVISE-010): 페이징 적용 후에도 행은 8컬럼(기사아이디/제목/작성자/수정자/작성시간/수정시간/기사상태/LockYN) 유지.
  it('AC-PAGE-6: 페이징 목록의 각 행이 8컬럼 구성을 유지한다 (회귀 가드)', async () => {
    const queryArticles = vi.fn().mockResolvedValue(twentyThreeRows);
    renderView(createFakeModel({ queryArticles }));
    await screen.findByText('bart-23');
    // 8컬럼 testid 가 행마다 존재한다 (페이지당 10행 × 8컬럼).
    expect(screen.getAllByTestId('article-id')).toHaveLength(10);
    expect(screen.getAllByTestId('article-author')).toHaveLength(10);
    expect(screen.getAllByTestId('article-modifier')).toHaveLength(10);
    expect(screen.getAllByTestId('article-time')).toHaveLength(10);
    expect(screen.getAllByTestId('article-edited-time')).toHaveLength(10);
    expect(screen.getAllByTestId('article-status')).toHaveLength(10);
    expect(screen.getAllByTestId('article-lockyn')).toHaveLength(10);
    // 헤더가 8컬럼 라벨을 모두 가진다.
    const header = screen.getByTestId('desk-header');
    for (const label of ['기사아이디', '제목', '작성자', '수정자', '작성시간', '수정시간', '기사상태', 'LockYN']) {
...
describe('ViewPage click → new window detail (news.md 기사 조회페이지)', () => {
  ...
  // AC-ROW-1 (SPEC-NEWS-REVISE-010): 행 (좌)클릭 → window.open 새창에 제목/본문/공통정보가 표시된다.
  // (위 'clicking a row opens a new window...' 테스트와 동일 동작을 AC-ROW-1 로 명시 잠금.)
  it('AC-ROW-1: 행 좌클릭이 window.open 새창에 제목/본문/공통정보를 쓴다', async () => {
    const u = userEvent.setup();
    const row = {
      articleId: 'A-ROW1', title: '행클릭 제목', content: '행클릭 본문',
      author: '작성행', status: 'RDS', createdAt: '2026-05-02T08:00:00Z',
    };
    const model = createFakeModel({ queryArticles: vi.fn().mockResolvedValue([row]) });
    const writeSpy = vi.fn();
    const fakeWindow = { document: { write: writeSpy, close: vi.fn() } };
    const openSpy = vi.spyOn(window, 'open').mockReturnValue(fakeWindow);
    try {
      renderView(model);
      await u.click(await screen.findByText('행클릭 제목'));
      expect(openSpy).toHaveBeenCalledWith('', '_blank', expect.stringContaining('width=720'));
      const html = writeSpy.mock.calls[0][0];
      expect(html).toContain('행클릭 제목'); // 제목
      expect(html).toContain('행클릭 본문'); // 본문
      expect(html).toContain('공통정보');   // 공통정보 섹션
      expect(html).toContain('작성행');     // 공통정보 필드
    } finally {
      openSpy.mockRestore();
    }
  });

  // AC-ROW-2 (SPEC-NEWS-REVISE-010): 행 좌클릭과 우클릭 → 상세보기가 동일 렌더 경로(buildArticleDetailHtml)
  // 를 사용해 동일 콘텐츠를 표시한다 (두 진입점 일관성 회귀 가드).
  it('AC-ROW-2: 행 클릭과 우클릭 상세보기가 동일한 상세 HTML(buildArticleDetailHtml)을 생성한다', async () => {
    const u = userEvent.setup();
    const row = {
      articleId: 'A-ROW2', title: '일관성 제목', content: '일관성 본문',
      author: '작성일관', status: 'RDS', createdAt: '2026-05-02T08:00:00Z',
    };
    // 동일 기사로 buildArticleDetailHtml 의 기준 출력을 계산한다.
    const expected = buildArticleDetailHtml(row);

    function captureFromEntry() {
      const writeSpy = vi.fn();
      const fakeWindow = { document: { write: writeSpy, close: vi.fn() } };
      const openSpy = vi.spyOn(window, 'open').mockReturnValue(fakeWindow);
      return { writeSpy, openSpy };
    }

    // (a) 행 좌클릭 경로.
    {
      const model = createFakeModel({ queryArticles: vi.fn().mockResolvedValue([row]) });
      const { writeSpy, openSpy } = captureFromEntry();
      try {
        renderView(model);
        await u.click(await screen.findByText('일관성 제목'));
    ...
    // (b) 우클릭 → 상세보기 경로.
    {
      ...
      try {
        renderView(model);
        const titleEl = await screen.findByText('일관성 제목');
        fireEvent.contextMenu(titleEl.closest('.yh-article-row'), { clientX: 100, clientY: 60 });
        const menu = screen.getByRole('menu', { name: '기사 메뉴' });
...
describe('ViewPage 컬럼 설정 진입점 — 헤더 우클릭 → 설정 → 중앙 모달 (2026-06-10 원안 복원)', () => {
