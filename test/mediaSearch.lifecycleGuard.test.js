...
// AC-MEDIA-1..4 — 영상 탭은 mediaSearch type 'video'(Youtube 전용), 이미지 탭은 type 'image'
// (Google 이미지 검색 전용), 글기사 탭은 내부 articleService.searchArticles (외부 API 미호출),
// 그리고 API 키가 응답에 절대 노출되지 않음을 단언한다.
//
// 2026-06-06 사용자 지시: 영상 탭=Youtube 결과, 이미지 탭=Google 이미지 검색 결과로 라우팅하며
// SPEC-NEWS-REVISE-002 D2-8 의 "Youtube-first, Google 폴백" 계약을 폐지(SUPERSEDE)한다 —
// 더 이상 공급자 간 폴백이 없다.
//
// 비고: SPEC 의 searchMedia({tab}) 예시 API는 실제 코드에 없다. 실제 아키텍처는
//   - 영상 탭   → createMediaSearchService({youtube, google}).search(query, 'video')  (Youtube 전용)
//   - 이미지 탭 → createMediaSearchService({youtube, google}).search(query, 'image')  (Google 전용)
...
// AC-MEDIA-1 (2026-06-06 type-split): type 미지정 → 'video' 정규화 → Youtube provider 1회 호출,
// Google 0회, 정규화 카드 페이로드. (이미지 탭의 Google 라우팅은 AC-MEDIA-2b가 별도 보장.)
test('AC-MEDIA-1: type 미지정 검색은 video로 정규화되어 Youtube만 1회 호출하고 정규화 카드를 반환한다', async () => {
...
// AC-MEDIA-2 (2026-06-06 type-split, supersedes D2-8 폴백): 영상 탭(type 'video')에서 Youtube가
// 실패하면 Google 폴백 없이 { items: [], error: true } 를 반환하고 Google provider는 호출되지 않는다.
test('AC-MEDIA-2: 영상 탭에서 Youtube가 실패하면 Google 폴백 없이 빈 결과+error를 반환한다', async () => {
  ...
  const result = await svc.search('올림픽', 'video');
  assert.equal(youtubeSearch.calls, 1, 'Youtube provider 호출됨');
  assert.equal(googleSearch.calls, 0, '폴백 폐지: Youtube 실패해도 Google은 호출되지 않는다');
  assert.equal(result.error, true);
  assert.deepEqual(result.items, []);
});

// AC-MEDIA-2b (2026-06-06 type-split): 이미지 탭(type 'image')은 Google만 호출하고 Youtube는 호출하지 않으며
// Google 정규화 카드를 반환한다.
test('AC-MEDIA-2b: 이미지 탭은 Google provider만 호출하고 Youtube는 호출하지 않으며 정규화 카드를 반환한다', async () => {
  const youtubeSearch = callCounter(async () => { throw new Error('이미지 탭에서 youtube가 호출되면 안 된다'); });
  const googleSearch = callCounter(async () => [
    { title: '올림픽 기사', url: 'https://g/1', thumbnailUrl: 'https://g/t1' },
  ]);
  const svc = createMediaSearchService({
  ...
  assert.equal(youtubeSearch.calls, 0, '이미지 탭은 Youtube를 호출하지 않는다');
  assert.equal(googleSearch.calls, 1, '이미지 탭은 Google provider를 1회 호출');
  assert.equal(result.error, false);
  assert.equal(result.items.length, 1);
  assert.deepEqual(result.items[0], {
    source: 'google',
