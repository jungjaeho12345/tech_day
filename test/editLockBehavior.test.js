...
// AC-EDIT-LOCK-6 (002) — applyAction 자체의 락 자동 검증. 종전에는 assertLockHolder 를 통한
// 서버 PUT 게이트만 단언했고 applyAction 직접 호출의 락 우회(action 라우트)는 미방어였다.
// 모든 케이스에서 now 고정 전달 — 실시간 시계 기준 30분 stale 판정 time-bomb 방지.
describe('AC-EDIT-LOCK-6 — applyAction 락 자동 검증 (편집/고침/포털고침 중 타 세션 차단)', () => {
  it('비보유자의 applyAction은 lock-required로 거부되고 status가 변경되지 않는다', () => {
    const { db, svc } = freshService();
    const articleId = seedArticle(svc);
    svc.acquireEditLock(articleId, { userId: 'U1', sessionId: 'P1', now: new Date('2026-06-04T01:00:00Z') });

    const result = svc.applyAction(articleId, 'D', 'send', {
      userId: 'U2', sessionId: 'P2', now: new Date('2026-06-04T01:05:00Z'),
    });
    assert.equal(result.ok, false);
  ...
  it('락 식별 정보 없는 applyAction도 타인 보유 락 앞에서는 거부된다 (호출자는 보유자일 수 없음)', () => {
    const { db, svc } = freshService();
    const articleId = seedArticle(svc);
    svc.acquireEditLock(articleId, { userId: 'U1', sessionId: 'P1', now: new Date('2026-06-04T01:00:00Z') });

    const result = svc.applyAction(articleId, 'D', 'send', { now: new Date('2026-06-04T01:05:00Z') });
    assert.equal(result.ok, false);
    assert.equal(result.reason, 'lock-required');
    assert.equal(db.prepare('SELECT status FROM Contents WHERE articleId = ?').get(articleId).status, 'RDS');
  });

  it('락 보유자 본인의 applyAction은 통과하고 전이가 적용된다', () => {
    const { db, svc } = freshService();
    const articleId = seedArticle(svc);
    svc.acquireEditLock(articleId, { userId: 'U1', sessionId: 'P1', now: new Date('2026-06-04T01:00:00Z') });

    const result = svc.applyAction(articleId, 'D', 'send', {
      userId: 'U1', sessionId: 'P1', now: new Date('2026-06-04T01:05:00Z'),
    });
    assert.equal(result.ok, true);
  ...
  it('락이 빈 기사의 applyAction은 락 컨텍스트 없이도 통과한다 (신규 작성 직후 송고 경로 보존)', () => {
    const { db, svc } = freshService();
    const articleId = seedArticle(svc);
    // 락 미획득 (lockYN='N') — 신규 작성 직후 송고와 동일한 상태.
    const result = svc.applyAction(articleId, 'R', 'send');
    assert.equal(result.ok, true);
    assert.equal(result.status, 'RDS', 'R/RDS/send → RDS (기자 송고는 RDS 유지)');
    assert.equal(db.prepare('SELECT lockYN FROM Contents WHERE articleId = ?').get(articleId).lockYN, 'N');
  });

  it('stale(30분 초과) 락은 해제된 것으로 간주되어 applyAction이 통과한다', () => {
    const { db, svc } = freshService();
    const articleId = seedArticle(svc);
    const T0 = new Date('2026-06-04T01:00:00Z');
    svc.acquireEditLock(articleId, { userId: 'U1', sessionId: 'P1', now: T0 });

    const Tstale = new Date(T0.getTime() + EDIT_LOCK_TIMEOUT_MS + 1000);
    const result = svc.applyAction(articleId, 'D', 'send', { userId: 'U2', sessionId: 'P2', now: Tstale });
    assert.equal(result.ok, true);
    assert.equal(db.prepare('SELECT status FROM Contents WHERE articleId = ?').get(articleId).status, 'DPS');
  });
});
