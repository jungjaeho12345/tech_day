...
> **AC 총수: 41** (acceptance.md §1~§16 실집계 — §17 SSOT 표기와 일치). 그중 [기구현/회귀가드] 38 + [테스트
> 공백/신설] 3(AC-DSN-2 / AC-UI-1 / AC-VW-2).
...
|----|----------------------|-----------|-----------|
| AC-DSN-2 | `web/src/view/ViewPage.statusBadge.test.jsx` | `--yh-badge-*` 색 토큰/`.yh-badge--*` 클래스 매핑(RDS=회색/송고=레드/보류=앰버/KILL=슬레이트) 단언 + 조회 목록 plain text 현행 고정(목록 배지 제거) | yonhap.css `--yh-badge-*` 토큰, `ViewPage.jsx:190-198` plain text 렌더 |
...
- AC-DSN-2: `ViewPage.statusBadge.test.jsx` — `--yh-badge-*` 색 토큰/`.yh-badge--*` 클래스 매핑(4 색 그룹) 단언
  + 조회 목록 상태 셀 plain text(목록 배지 제거) 현행 고정.
